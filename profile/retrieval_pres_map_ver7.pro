...
; retrieval_pres_map_ver6　 ::2023.07.07 Fri 12:04:00
; ver6: 等価幅法でQLを作成するためのプログラム
; LUT変更に伴う圧力マップへの影響を確認するプログラム
;
; retrieval_pres_map_ver7　 ::2023.07.07 Fri 18:30:00
; ver7: 等価幅法でQLを作成するためのプログラム
; LUT7を使用して、リトリーバルをし直す
; 
;------------------------------------------------------------------------------------------------------------------------
...
Pro retrieval_pres_map_ver7

; =========== path ===========
pathfile = '/data2/omega/sav/'
path_work = '/work1/LUT/SP/EWwork_LUT7/'
path_ql = '/work1/LUT/SP/QL_LUT7/'
pathwork = '/work1/LUT/SP/EWwork/'
path_good = '/work1/LUT/SP/'
path_dust = '/work1/LUT/dust/table/index/'

files=file_search(path_work+'*.sav',count=count) ; sav fileすべてを持ってくる

loop_b = where(files eq '/work1/LUT/SP/EWwork_LUT7/EW_work_ORB0923_3.sav') ; MY27 Ls 0
loop_b = fix(loop_b(0))
...
; ++++++ restore file ++++++
  ...
  ; work file restore
  MCDpressure = MCDpressure
  dustmap = dustmap
  TAmap = TAmap
  TBmap = TBmap
  InputAlbedo = InputAlbedo
  ...
  file_ew = pathwork + 'EW_work_' + fileorbit + '.sav'
  restore, file_ew

  MOLAaltitude = altitude

  file_sav = pathfile + fileorbit + '.sav'
  restore, file_sav

  ; restore dust index
  file_dust = path_dust + fileorbit + '.sav'
  restore, file_dust
  dust_CD = dust_CD
...
; ++++++++ retrieval準備 ++++++++++
  ...
  ; (8,17,24,27)
  ; 経年劣化で使用しないスペクトルを決める + LUT 7に合わせたスペクトルを選択
  ;jdat(*,1,*)= !VALUES.F_NAN
  ;jdat(*,2,*)= !VALUES.F_NAN
  ;jdat(*,5,*)= !VALUES.F_NAN
  ;jdat(*,6,*)= !VALUES.F_NAN
  ;jdat(*,7,*)= !VALUES.F_NAN
  jdat(*,8,*)= !VALUES.F_NAN
  ;jdat(*,11,*)= !VALUES.F_NAN
  jdat(*,17,*)= !VALUES.F_NAN
  ;jdat(*,18,*)= !VALUES.F_NAN
  ;jdat(*,19,*)= !VALUES.F_NAN
...
; +++++++ MCD v5.3 data input file +++++++
  ...
  ; ++++++++++++ dust indexを使用するための準備 ++++++++++++++++++
  min_lati = fix(min(lati))
  max_lati = fix(max(lati)) + 1
  dev_dust = 1
  def_dust = max_lati - min_lati
...
; =========== bad spectrum skip =========== 
      ...
      PA = PA_all(l,k)
      TA = TAmap(l,k)
      ;dust_opacity = dustmap(l,k)
      TB = TBmap(l,k)
      altitude = MOLAaltitude(l,k)

      ; dust indexをrestoreして、ダストを決める !TBD ここは早くかけそう！
      for loop_k = 0, def_dust do begin 
        if lati(l,k) gt min_lati + loop_k and lati(l,k) lt min_lati + loop_k + dev_dust then dust_opacity = dust_CD(loop_k)
      endfor

      ; reflectance factor ( I/F / cos(SZA) )
      Albedo_input = InputAlbedo(l,k)
...
; =========== retrieval skip ===========
      ...
      ;if input_fileorbit ge 0181 and fileorbit le 2595 then d_index = 27
      ;if input_fileorbit ge 2607 and fileorbit le 5055 then d_index = 28
      ;if input_fileorbit ge 5062 and fileorbit le 7454 then d_index = 29
      ...
      ;scena = 28       ; <integer> scenario (1 = Climatology ave solar)
...
; +++++++++ retrieval ++++++++++++
      pressure(l,k) = ret_pressure(trans, TA, TB, SZA, EA, PA, dust_opacity, ice_opacity, Albedo_input)
...
;  +++++++++ 各種indexを計算 ++++++++++++
  ...
  ; =========== save work file ===========
  ; create work file
  save, lati, longi, MOLAaltitude,  pressure, InputAlbedo, dustmap, TAmap,TBmap, MCDpressure, SZA_all, EA_all, PA_all, local_time, SOLAR_LONGITUDE, p1_slev, bad_index, bad_ratio, SZA_ratio, EA_ratio, dust_ratio, albedo_high_ratio, albedo_low_skip, filename = path_dust + 'EW_work_' + fileorbit + '.sav'
  ...
  Write_JPEG, path_dust + 'SP_' + fileorbit +'.jpg', snapshot, True=1, Quality=75

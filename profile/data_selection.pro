...
; CO2吸収量からpressure retrieval data selectionのためのコード
; @author: A. Kazama kazama@pparc.gp.tohoku.ac.jp
; Created on Wed Oct 19 17:02:00 2022
; クリアスカイに近いような状況を選択
...
files=file_search(pathfile+'ORB'+'*.sav',count=count)
force=1

for loop=0,count-1 do begin
; for loop=0,20 do begin
...
; colorMOLAをMOLAのmappingに使用
  ...
  ; colordevがMOLA - co2吸収量の差をmapping
  ; colordev= 120:140の範囲のピクセルを数える
  plotdev = double((newcolor-minvalue)/(maxvalue-minvalue))*255
  colordev = plotdev
  
  
  ; ========= 差が120:140の範囲を探索し、5割以上なら格納 ===========
  ; ind_dev = where(colordev gt 100 and colordev lt 160)
  ind_dev = where(colordev gt 120 and colordev lt 140)
  
  ; ratioがあまりうまく評価できていない、、、どうしよう、、、
  element_ind = double(n_elements(colordev))
  element_dev = double(n_elements(ind_dev))
  ratio = double(element_dev/element_ind)
  
  print, 'raito:',ratio
  print, 'ORB number:', fileorbit
  if ratio gt 0.5 then begin
 ...
 ; ========= 他の軌道データについて =============
    ...
    Write_JPEG, path_save+fileorbit+'_select.jpg', snapshot, True=1, Quality=100
    
    print, 'ratio 0.5以上'
        
  endif
  if ratio lt 0.5 then goto, skip
  skip:

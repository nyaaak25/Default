...
;
; tidalwave_analysis_v5_3　 ::2023.7.12 Wed 16:37:00
; 熱潮汐波の検出をする
; function化 + Ls indexを付加
; 平均方法：各スペクトルで中央値を探す
; データセレクション後のものだけを使用する
; !TBD! albedo, water ice, dust index use
; 
;
;------------------------------------------------------------------------------------------------------------------------

function data_selection, index, local, dev_local, min_latitude, max_latitude
...
path_work = '/work1/LUT/SP/EWwork_good/'
...
for loop = 0, n_loop_index -1 do begin
    ...
    ind_LT = where(localtime gt local and localtime lt local + dev)
    if ind_LT(0) eq -1 then goto, skip1
...
file = path_common + 'Ls_good_index.sav'
restore, file
...
for i = 12, 24d, dev_local do begin
    str_i = strupcase(strmid(i,1,4,/REVERSE_OFFSET))
    index = where(ls_ind ge min_Ls and ls_ind le max_Ls)
    ...
    spectrum = data_selection(index, i, dev_local, min_latitude, max_latitude)

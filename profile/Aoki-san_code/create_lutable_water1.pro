...
;------------------------------------------------------------------------------------------------------------------------
Pro Create_lutable_water1
;------------------------------------------------------------------------------------------------------------------------

;cd, '/home/nyonnkazama/' ;working dir
cd, '/home/shoheiaoki/work/' ;working dir
path_LUT_Input = '/work1/LUT/H2O/table/input/'
path_LUT_Output = '/work1/LUT/H2O/table/output/'

wl = dblarr(11)
openr, lun, path_LUT_Input+'wl_grid_water.dat', /get_lun
for i = 0, 10 do begin
  readf, lun, a
  wl(i) = a
endfor
...
openw, lun, path_LUT_Input+'wn_grid_water.dat', /get_lun
...
for IT1 = 1, 5 do begin    ;2) for T1
    for IT2 = 1, 3 do begin  ;3) for T2
      ...
      For i = 0, 50 do begin
        TEMP(i) = a*HIGH(i) + b
        IF(HIGH(i) ge H*4.0) then TEMP(i) = T2
      endfor

      kfike_H2O = path_LUT_Input + 'H2O_SP' + STRCOMPRESS(ISP, /REMOVE_AL) + '_TA' + STRCOMPRESS(IT1, /REMOVE_AL) + '_TB'  + STRCOMPRESS(IT2, /REMOVE_AL) + '.k'
      kfike_CO2 = path_LUT_Input + 'CO2_SP' + STRCOMPRESS(ISP, /REMOVE_AL) + '_TA' + STRCOMPRESS(IT1, /REMOVE_AL) + '_TB'  + STRCOMPRESS(IT2, /REMOVE_AL) + '.k'
      atmfile = path_LUT_Input + 'SP' + STRCOMPRESS(ISP, /REMOVE_AL) + '_TA' + STRCOMPRESS(IT1, /REMOVE_AL) + '_TB'  + STRCOMPRESS(IT2, /REMOVE_AL) + '.atmos'
            ...
            for ID = 1, 6 do begin ;7) Dust
              ...
              for IWI = 1, 3 do begin ;8) Water ice 
                ...
                for IAB = 1, 6 do begin ;9) Surface albedo
                  ...
                  for Iwater = 100, 0, -1 do begin ;1) for water vapor
                    output_name = path_LUT_Output + 'Water' + STRCOMPRESS(Iwater,/REMOVE_AL) $
                      ...
                      + '_WaterI' + STRCOMPRESS(IWI,/REMOVE_AL) $
                      + '_SurfaceA' + STRCOMPRESS(IAB,/REMOVE_AL) + '.dat'


                    ;make profile_tmp.atmos
                    ;1pr-um = 3.34d18 1/cm2, calculated by ideal gas law
                    prum = 3.34d18
                    total_number_density = 0d
                    for i = 0, 50 do total_number_density = total_number_density + PRES(i)/(Temp(i) * Kb) * 1d-6
                    total_number_density = total_number_density * 1d5 ; 1km in cm
                    VMR = (3.34d18 * double(Iwater)) / total_number_density

                    output_name_rad = path_LUT_Output + 'Water' + STRCOMPRESS(fix(Iwater),/REMOVE_AL) + '_SP' + STRCOMPRESS(fix(ISP),/REMOVE_AL) $
                    ...
                    output_name_rad_mono = path_LUT_Output + 'Water' + STRCOMPRESS(fix(Iwater),/REMOVE_AL) + '_SP' + STRCOMPRESS(fix(ISP),/REMOVE_AL) $
                                                            + '_TA' + STRCOMPRESS(fix(IT1),/REMOVE_AL) $
                                                            + '_TB' + STRCOMPRESS(fix(IT2),/REMOVE_AL) $
                                                            + '_SZA' + STRCOMPRESS(fix(ISZA),/REMOVE_AL) $
                                                            + '_EA' + STRCOMPRESS(fix(IEA),/REMOVE_AL) $
                    ...
                    output_name_trans = path_LUT_Output + 'Water' + STRCOMPRESS(fix(Iwater),/REMOVE_AL) + '_SP' + STRCOMPRESS(fix(ISP),/REMOVE_AL) $
                                                            + '_TA' + STRCOMPRESS(fix(IT1),/REMOVE_AL) $
                                                            + '_TB' + STRCOMPRESS(fix(IT2),/REMOVE_AL) $
                                                            + '_SZA' + STRCOMPRESS(fix(ISZA),/REMOVE_AL) $
                                                            + '_EA' + STRCOMPRESS(fix(IEA),/REMOVE_AL) $
                    ...
                    output_name_trans_mono = path_LUT_Output + 'Water' + STRCOMPRESS(Iwater,/REMOVE_AL) + '_SP' + STRCOMPRESS(fix(ISP),/REMOVE_AL) $
                                                            + '_TA' + STRCOMPRESS(fix(IT1),/REMOVE_AL) $
                                                            + '_TB' + STRCOMPRESS(fix(IT2),/REMOVE_AL) $
                                                            + '_SZA' + STRCOMPRESS(fix(ISZA),/REMOVE_AL) $
                                                            + '_EA' + STRCOMPRESS(fix(IEA),/REMOVE_AL) $
                    ...
                    printf,lun,"'/work1/data/ARS_example/Status.sta' = .File_Status. ! debug and multiple processes run control purpose"
                    printf,lun,"0 = .Intel_VML. ! 0 or 1"
                    printf,lun,""                                                                                                                              
                    printf,lun,"===   Output Files   ==="                                                                                                      
                    printf,lun,""                                                                                                                              
                    printf,lun,"'"+output_name_trans+"'  = .File_Transmittance."                                                                                                  
                    printf,lun,"'"+output_name_trans_mono+"'  = .File_Monochromatic_Transmittance. "                                                                                    
                    printf,lun,"'"+output_name_rad+"'  = .File_Radiance."
                    printf,lun,"'"+output_name_rad_mono+"' = .File_Monochromatic_Radiance."                                                                                          
                    printf,lun,""                                                                                                                              
                    printf,lun,"===   Wavenumber range and radiative transfer option   ================"                                                       
                    printf,lun,""                                                                                                                              
                    printf,lun," 3750  = .Vmin."                                                                                                                 
                    printf,lun," 4000  = .Vmax."           
                    ...
                    printf,lun,""                                                                                                                              
                    printf,lun,"'/work1/LUT/H2O/table/input/LUtable.v' = .File_Wavenumber_Grid."                    
                    printf,lun,""                                                                                                                              
                    printf,lun,"=====   Gases   ========================================================"                                                     
                    printf,lun,""                                                                                                                              
                    printf,lun,""                                                                                                                              
                    if Iwater ne 0 then printf,lun," 2 = .Number_of_Species. "                                                                                                      
                    if Iwater eq 0 then printf,lun," 1 = .Number_of_Species. "
                    printf,lun,""
                    if Iwater ne 0 then begin                                                                                                                              
                    printf,lun," 1 0 1                                  ! molecule and isotope codes, add.broad."
                    printf,lun,"' '          ! spectral lines file name"
                    printf,lun,"'/work1/LUT/H2O/table/input/LUtable.v'                ! specific wavenumber grid file name"
                    printf,lun,"'"+kfike_H2O+"'          ! absorption coefficients file name"
                    printf,lun," "+STRCOMPRESS(float(VMR),/REMOVE_ALL)+" ' ' 1. "
                    ...
                    printf,lun,"' '          ! spectral lines file name"
                    printf,lun,"'/work1/LUT/H2O/table/input/LUtable.v'                ! specific wavenumber grid file name"
                    printf,lun,"'"+kfike_CO2+"'          ! absorption coefficients file name"
                    printf,lun," 0.9532 ' ' 1. "
                    printf,lun," 1 0 0 0 0                                        ! line shape"                                                                 
                    printf,lun,""                                                                                                                              
                    printf,lun,""                                                                                                                              
                    ...
                    printf,lun," 2 3838.0 "+string(dust,format='(f6.4)')+" "+string(H*1e-3,format='(f5.2)')+" 0 50" 
                    printf,lun,""                                                                                            
                    printf,lun," 3 '/work1/LUT/Common/waterice_2500_8500.aero'"                                                                                                                             
                    printf,lun," 2 3838.0 "+string(waterice,format='(f6.4)')+" "+string(H*1e-3,format='(f5.2)')+" 0 50" 
                    printf,lun,""
                    ...
                    printf,lun," 19.19D+0 = .Resolution.    [Ideal PFS: Sinc:1.211376319487D+0, Hamming:1.822245687D+0]"                                         
                    printf,lun,"                    [IRIS: Hamming => 2.1254448]"                                                                              
                    printf,lun,""                                                                                                                              
                    printf,lun," -1 = .Spectral_Step."                                                                                                          
                    printf,lun,"'/work1/LUT/H2O/table/input/wn_grid_water.dat' = .File_Spectral_Channels.  ! if Spectral_Step < 0"                            
                    printf,lun,""                                                                                                                              
                    ...
                    ;run ARS
                    ;------------------------------------------------------------------------------------------------------------------------
                    spawn,'/work1/bin/arsm arsm_tmp1.ini'

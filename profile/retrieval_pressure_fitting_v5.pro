...
;-------------------------------------------------------------------------------
function multifunction, TA, TB, SZA, EA, PA, Dust, Waterice, Albedo
;-------------------------------------------------------------------------------
T1 = TA
T2 = TB

;note: y_tmp =n_elements(pressure, wavelength)
y_tmp = dblarr(15, 29)
...
restore, '/work1/LUT/Common/fitting_ytmp.sav'
nx = n_elements(x)
y = dblarr(nx)
...
Pro retrieval_pressure_fitting_v5
...
for l = ip_b, ip_a -1 do begin ;loop for slit scan
  for k = io_b, io_a -1 do begin ;test getting surface feature
    ...
    ;y_obs = jdat(l,CO2, points(k))
    y_obs = y_obs/specmars_CO2

    y_tmp = multifunction(TA, TB, SZA, EA, PA, dust_opacity, ice_opacity, Albedo_input)
...
save, lati,longi,pressure,filename='/work1/LUT/SP/table/absorption/pressuremap_ORB0931_3.sav'

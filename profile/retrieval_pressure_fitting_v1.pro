...
; continuum決定のためのガウシアン足し合わせ
;---------------------------
function pyroxenes, x, p
;---------------------------
; 中心波長:1.9 μm、FWHM:0.5 μm
y1 = 1d - gauss1(x, [1.9d, 0.5d / (2 * sqrt(2*alog(2))), P(0)], /peak)
; 中心波長:2.3 μm、FWHM:0.56 μm
...
; ARS dataを読み出すfunction
;---------------------------
function forward, x, p
;---------------------------
;x: wavenumber
;p: 9 paprameters, surface pressure, T1, T2, SZA, EA, PhaseA, Dust, Water ice clouds, Albedo, continuum fitting function
;return: radiance
;---------------------------
...
for i = 0, 15-1 do begin
  for j = 0, nx-1 do begin
    ...
    if j eq 0 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad0.sav'
    if j eq 1 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad1.sav'
    if j eq 2 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad2.sav'
    if j eq 3 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad3.sav'
    if j eq 4 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad4.sav'
    if j eq 5 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad5.sav'
    if j eq 6 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad6.sav'
    if j eq 7 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad7.sav'
    if j eq 8 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad8.sav'
    if j eq 9 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad9.sav'
    if j eq 10 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad10.sav'
    if j eq 11 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad11.sav'
    if j eq 12 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad12.sav'
    if j eq 13 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad13.sav'
    if j eq 14 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad14.sav'
    if j eq 15 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad15.sav'
    if j eq 16 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad16.sav'
    if j eq 17 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad17.sav'
    if j eq 18 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad18.sav'
    if j eq 19 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad19.sav'
    if j eq 20 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad20.sav'
    if j eq 21 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad21.sav'
    if j eq 22 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad22.sav'
    if j eq 23 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad23.sav'
    if j eq 24 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad24.sav'
    if j eq 25 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad25.sav'
    if j eq 26 then restore,'/work1/LUT/SP/table/LUT_fitting_old/Table_calc_wave_rad26.sav'
...
; CO2 absorption bandに対応したspecmarsを呼び出して、それでI/Fの計算をする
restore, '/work1/LUT/Common/specmars_CO2.sav'
Y = Y/specmars

; continuum fitting (2つのガウシアンの足し合わせ)
...
Pro retrieval_pressure_fitting_v1
; fitting test
...
; continuum fittingのための波長とI/F
; SWIR　channelを全部使用
; OMEGAの死んだ素子をnanに置き換え
x0 = reform(wvl(0:127))
...
; continuum fitting
need_ind = where(x0 ge 1.2 and x0 le 2.6 )
pi = replicate({step:0d, fixed:0, limited:[0,0], limits:[0.D,0.D]}, 3)
start = [0.06, 0.06, median(y0(need_ind))]
Result_Fit0 = MPFITFUN('pyroxenes', x0(need_ind), y0(need_ind), y0(need_ind)*1d-2, start, PARINFO=pi, MAXITER=20, BESTNORM=BESTNORM0, MPSIDE=2, status=status, yfit=yfit, /nan)
F0 = yfit
...
x =wvl[CO2]
y =reform(jdat(xind(0),CO2,yind(0)))
specmars = specmars(CO2)
y = y/specmars

nanserch=where(y ge 0 and y le 0.0001)
...
; 圧力だけを振ったLUTを使ってMPFITFUNを使う、毎回多次元補間をしなければいいんじゃないか。
F = yfit

...
for j = 0, n-1 do begin
  ...
  xmin=1.e38
  k=0
  for i = 0, n+1-j-1 do begin
    if(abs(x(i)) lt xmin) then begin
      xmin=abs(x(i))
      k=i
    endif
    if(x(i) ne 0) then y(i) = (y(i)-cof(j))/x(i)
  endfor
  for i = k, n+1-j-1 do begin
    y(i-1)=y(i)
    x(i-1)=x(i)
  endfor
endfor
...
pro polin2, x1a, x2a, ya, m, n, x1, x2, y, dy
...
for j = 0, m-1 do begin
  ...
  polcof, xa2, yntmp, n, x2, ymtmp(j), dy
endfor
polcof, x1a, ymtmp, m, x1, y, dy
...
restore, '/work1/LUT/Common/fitting_ytmp_SA.sav'
nx = n_elements(x)
y = dblarr(nx)
...
for I = 0, 7-1 do begin
  F = Albedo_grid(I) - P(1)
  if (F ge 0.0d0) then j2 = I
  if (F ge 0.0d0) then goto, skip02
endfor
skip02:

if P(1) gt 0.6 then stop
if P(1) lt 0.05 then stop
print, p(1)
j1 = j1 -1
j2 = j2 -1
...
;Y = Y * ( pyroxenes(x, P(1:3)) / mean(pyroxenes(x, P(1:3))) )
...
Pro retrieval_pressure_fitting_v3_SA
...
;window,0
;plot, x0, y0, back=255, color=0, xs=1, ys=1, thick=3, psym=8;, yr=[0.11, 0.13]
;oplot, x0(good), F0, color=254, thick=3
;oplot, x0(good), y0(good), psym=8, symsize=2, color=100
;oplot, x0, y0, color=0
;stop
...
for i = 0, 15-1 do begin
  for j = 0, nx-1 do begin
    ...
    y_tmp(i,j,j8) = y11111111*(1.d - t1)*(1.d - t2)*(1.d - t3)*(1.d - t4)*(1.d - t5)*(1.d - t6)*(1.d - t7)*(1.d - t8) +   $
...
;      print, t1, t2, t3, t4, t5, t6, t7, t8
      ;print, i, j, j8, y_tmp(i,j,j8), y11111111
;      stop
...
save, y_tmp, filename='/work1/LUT/Common/fitting_ytmp_SA.sav'
stop
...
start = [SP, Albedo, 1d, 0d];, 0d]
;pi(1).fixed = 1 ;albedo

;2 gaussiun fitting
;pi = replicate({step:0d, fixed:0, limited:[0,0], limits:[0.D,0.D]}, 4)
;start = [SP, Result_Fit0(0), Result_Fit0(1), Result_Fit0(2)]
...
plot, x(good), y_obs(good), yr=[-0.1,0.3], back=255, color=0, thick=3, psym=-1, xr=[1.85, 2.2], xs=1

...
; 等価幅法でQLを作成するためのプログラム
; 条件が悪いところを除いて、全部のORBに適応するようにプログラムを改変する
; limited: EA<30, SZA<55, 極域(70°以南/以北)、LUTの軸を超えた値ならNaNが入るようにする
; *TBD* dust factorを追加、topography correctionを追加、T1を追加
; 
;------------------------------------------------------------------------------------------------------------------------
...
for loop = loop_b, loop_a do begin
  ...
  ; create work file
  save, lati, longi, altitude, trans, pressure, InputAlbedo, dustmap, TAmap, MCDpressure, SZA_all, EA_all, filename = path_work + 'EW_work_' + fileorbit + '.sav'

  ; なんらかのskipに引っかかってimageの100%Retrievalされなかった場合は、そのQLを作らない
  ...
  if skip_ratio gt 97d  then begin
    ...
    print, 'WARNING: Could not create Work file'
    goto, skip10
  endif

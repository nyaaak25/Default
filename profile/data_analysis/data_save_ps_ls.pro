...
mcd_ps_ind = dblarr(loop_a - loop_b +1)
mcd_ps_ind_12 = dblarr(loop_a - loop_b +1)
...
for loop = loop_b, loop_a do begin
    ...
    for l = ip1, ip2 -1 do begin ;loop for slit scan
        for k = io1, io2 -1 do begin ;test getting surface feature
            ;if p10(l,k) eq 0 then print, 'for loop skip'
    ...
    for i = 0, ip-1 do for j = 0, io-1 do if mcd_ps_12(i,j) gt 0 then p2(i,j) = mcd_ps_12(i,j) * exp( (p5(i,j)) / (R*mcd_ta_12(i,j)/(-Gconst*MMars/(-1d*(RMars+p5(i,J))*(RMars+p5(i,J)) ))))
    ...
    ls_ind(loop) = ls
    mcd_ps_ind(loop) = various_p1
    mcd_ps_ind_12(loop) = various_p2
    lt_ind(loop) = localtime

    print, "loop:", (loop - loop_b) + 1 , '/', (loop_a - loop_b) + 1
    ;print, "LT12 ps:", various_p2
    ;print, "ps:", various_p1
    print, "div: ", various_p1 - various_p2

endfor
save, ls_ind, mcd_ps_ind, lt_ind, mcd_ps_ind_12, filename = path_work + 'moving_latitude_' + str_min + '_' + str_max + '_new.sav'
stop

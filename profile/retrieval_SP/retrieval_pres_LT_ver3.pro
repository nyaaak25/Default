;------------------------------------------------------------------------------------------------------------------------
;
; ある1点の場所でEquivalent width法を用いて気圧値をretireivalするプログラム
;
; create by Akira Kazama
; 
; retrieval_pres_LT_ver2　 ::2022.10.31 Mon 11:43:00
; ver2: LUT update後に対応するプログラム
;
; retrieval_pres_LT_ver3　 ::2023.02.22 Wed 14:01:00
; ver3: EW法で特定地点での圧力をリトリーバルする
; 
;------------------------------------------------------------------------------------------------------------------------
...
Y = [jdat(xind,0,yind), jdat(xind,1,yind), jdat(xind,2,yind), jdat(xind,23,yind), jdat(xind,25,yind), jdat(xind,26,yind)]
coef = linfit(X,Y)
...
; create Albedo input
Albedo_input1 = jdat(xind,0,yind)/specmars(0) / cos(geocube(xind,8,yind)*1e-4*!DTOR)
Albedo_input2 = jdat(xind,1,yind)/specmars(1) / cos(geocube(xind,8,yind)*1e-4*!DTOR)
Albedo_input3 = jdat(xind,25,yind)/specmars(25) / cos(geocube(xind,8,yind)*1e-4*!DTOR)
Albedo_input4 = jdat(xind,26,yind)/specmars(26) / cos(geocube(xind,8,yind)*1e-4*!DTOR)

Albedo_input = (Albedo_input1 + Albedo_input2 + Albedo_input3 + Albedo_input4) / 4
